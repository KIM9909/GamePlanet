...
public class BluemarbleGameController {
	...
	public void choosePosition(@DestinationVariable("roomId") int roomId,
			@Payload ChoosePositionRequest request) {
		SocketResponse<ChoosePositionResponse> response = SocketResponse.from("choose-position",
				bluemarbleGameService.choosePosition(roomId, request), "시간여행을 합니다!");
		messagingTemplate.convertAndSend("/topic/rooms/" + roomId, response);
	}

	@MessageMapping("/{roomId}/game-end")
	public void gameEnd(@DestinationVariable("roomId") int roomId) {
		SocketResponse<String> response = SocketResponse.from("game-end",
				bluemarbleGameService.gameEnd(roomId), "게임이 종료되었습니다.");
		messagingTemplate.convertAndSend("/topic/rooms/" + roomId, response);
	}
}